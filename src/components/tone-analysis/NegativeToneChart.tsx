...
interface NegativeToneChartProps {
  fromDate?: string;
  toDate?: string;
  agentId?: string;
}

export function NegativeToneChart({ 
  fromDate: propFromDate, 
  toDate: propToDate, 
  agentId 
}: NegativeToneChartProps) {
  ...
  const { data: apiData, isLoading, error } = useToneAnalysis({ 
    isPositive: false,
    fromDate,
    toDate,
    agentId
        ...
        <TooltipProvider delayDuration={0}>
          <Tooltip>
            ...
            <TooltipContent className="bg-[#ffffff] text-[#6e6e6e]">
              Analysis of negative communication tone across different
              attributes
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
