...
export async function GET(request: Request) {
  try {
    ...
    const url = `http://api.avaflow.net/tone/agent/analysis?${apiParams.toString()}`;
    const response = await fetch(url);
    
    if (!response.ok) {
      throw new Error('Failed to fetch agent analysis data');
    }
    const data = await response.json();
    return NextResponse.json(data);
  } catch (error) {
    return NextResponse.json(
      { error: 'Failed to fetch agent analysis data' },
      { status: 500 }
    );
  }
}
